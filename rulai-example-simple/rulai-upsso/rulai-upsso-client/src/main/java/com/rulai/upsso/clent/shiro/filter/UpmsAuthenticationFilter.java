package com.rulai.upsso.clent.shiro.filter;
...
public class UpmsAuthenticationFilter extends AuthenticationFilter {
    ...
    // 局部会话key
    private final static String RULAI_UPSSO_CLIENT_SESSION_ID = "rulai-sso-client-session-id";
    // 单点同一个code所有局部会话key
    private final static String RULAI_UPSSO_CLIENT_SESSION_IDS = "rulai-sso-client-session-ids";

    @Autowired
    UpmsSessionDao upmsSessionDao;
    
    private final static  String  RULAI_UPSSO_CLIENT_PROPERTIES="rulai-upsso-client"; 
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        // 判断请求类型
        String upmsType = PropertiesFileUtil.getInstance(RULAI_UPSSO_CLIENT_PROPERTIES).get("rulai.upsso.type");
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        StringBuffer sso_server_url = new StringBuffer(PropertiesFileUtil.getInstance(RULAI_UPSSO_CLIENT_PROPERTIES).get("rulai.upsso.sso.server.url"));
        // server需要登录
        String upmsType = PropertiesFileUtil.getInstance(RULAI_UPSSO_CLIENT_PROPERTIES).get("rulai.upsso.type");
        if ("server".equals(upmsType)) {
        ...
        sso_server_url.append("/sso/index").append("?").append("appid").append("=").append(PropertiesFileUtil.getInstance("rulai-sso-client").get("rulai.upsso.appID"));
        // 回跳地址
    ...
    private boolean validateClient(ServletRequest request, ServletResponse response) {
        ...
        // 判断局部会话是否登录
        String cacheClientSession = RedisUtil.get(RULAI_UPSSO_CLIENT_SESSION_ID + "_" + session.getId());
        if (StrUtil.isNotBlank(cacheClientSession)) {
            // 更新code有效期
            RedisUtil.set(RULAI_UPSSO_CLIENT_SESSION_ID + "_" + sessionId, cacheClientSession, timeOut);
            Jedis jedis = RedisUtil.getJedis();
            jedis.expire(RULAI_UPSSO_CLIENT_SESSION_IDS + "_" + cacheClientSession, timeOut);
            jedis.close();
            // 移除url中的code参数
            ...
            try {
                ...
                if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                    ...
                    if (1 == result.getIntValue("code") && result.getString("data").equals(code)) {
                        // code校验正确，创建局部会话
                        RedisUtil.set(RULAI_UPSSO_CLIENT_SESSION_ID + "_" + sessionId, code, timeOut);
                        // 保存code对应的局部会话sessionId，方便退出操作
                        RedisUtil.sadd(RULAI_UPSSO_CLIENT_SESSION_IDS + "_" + code, sessionId, timeOut);
                        _log.debug("当前code={}，对应的注册系统个数：{}个", code, RedisUtil.getJedis().scard(RULAI_UPSSO_CLIENT_SESSION_IDS + "_" + code));
                        // 移除url中的token参数
                        String backUrl = RequestParameterUtil.getParameterWithOutCode(WebUtils.toHttp(request));
                        // 返回请求资源
                        try {
